...
function LoadingState({ type = "dashboard", rows = 4, className = "" }) {
  if (type === "card") {
    return (
      <div className={`p-4 space-y-3 animate-pulse ${className}`} role="status">
  ...
  if (type === "inline" || type === "simple") {
    return (
      <div className={`flex items-center justify-center p-6 space-x-2 animate-pulse ${className}`} role="status">
  ...
  if (type === "table" || type === "section") {
    return (
      <div className={`w-full space-y-3 p-2 animate-pulse ${className}`} role="status">
        {Array.from({ length: Math.max(3, rows) }).map((_, i) => (
          <div key={i} className="flex items-center space-x-4 py-2.5 border-b border-ink-100 last:border-0">
            <div className="w-8 h-8 bg-ink-200 rounded-full shrink-0" />
            <div className="flex-1 space-y-2">
              <div className="h-3.5 bg-ink-200 rounded w-1/3" />
  ...
  return (
    <div className={`w-full space-y-6 animate-pulse ${className}`} role="status">
      {/* Cards Grid Skeleton */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="p-4 bg-white border border-ink-100 rounded-xl space-y-3">
            <div className="flex justify-between items-center">
              <div className="h-3.5 w-24 bg-ink-100 rounded" />
              <div className="w-6 h-6 bg-ink-100 rounded-full" />
            </div>
            <div className="h-6 w-20 bg-ink-200 rounded" />
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-ink-100 rounded-xl p-5 space-y-4">
        <div className="space-y-3">
          {Array.from({ length: Math.max(3, rows) }).map((_, i) => (
            <div key={i} className="flex items-center space-x-4 py-3 border-b border-ink-100 last:border-0">
              <div className="w-8 h-8 bg-ink-200 rounded-full shrink-0" />
              <div className="flex-1 space-y-2">
