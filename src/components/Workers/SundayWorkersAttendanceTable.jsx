...
import { getTeamForDepartment, routeObject } from "../../utils/routeObject";
...
function buildTableRows(rows) {
  ...
  items.forEach((item) => {
    ...
    const team = resolveTeamForDepartment(dept) || "Other";
  ...
  return (
    <div className="mt-8 overflow-x-auto rounded-lg border border-gray-300">
      <table className="min-w-full border-collapse border border-gray-300 text-sm">
        <caption className="bg-orange-500 py-3 text-center text-lg font-bold text-white">
          {titleDate} - SUNDAY WORKERS ATTENDANCE
        </caption>
        <thead>
          <tr>
            <th className="border border-gray-300 bg-orange-500 px-3 py-2.5 text-left font-bold text-white">
              DIRECTORATE
            </th>
            <th className="border border-gray-300 bg-orange-500 px-3 py-2.5 text-left font-bold text-white">
              TEAMS
            </th>
            <th className="border border-gray-300 bg-orange-500 px-3 py-2.5 text-center font-bold text-white">
              TEAM STRENGTH
            </th>
            <th className="border border-gray-300 bg-green-600 px-3 py-2.5 text-center font-bold text-white">
              Present
            </th>
            <th className="border border-gray-300 bg-green-600 px-3 py-2.5 text-center font-bold text-white">
              % of Present
            </th>
            <th className="border border-gray-300 bg-rose-600 px-3 py-2.5 text-center font-bold text-white">
              Absent
            </th>
          </tr>
        </thead>
        <tbody>
          {rowsWithSpan.map((row, idx) => (
            <tr key={idx}>
              {row.directorateRowSpan > 0 ? (
                <td
                  className={`border border-gray-300 px-3 py-2 align-middle ${DIRECTORATE_COLORS[row.directorate] || ""}`}
              ...
              <td
                className={`border border-gray-300 px-3 py-2 ${DIRECTORATE_COLORS[row.directorate] || ""}`}
              >
                {row.teamLabel}
              </td>
              <td className="border border-gray-300 bg-amber-100 px-3 py-2 text-center">
                {row.total}
              </td>
              <td className="border border-gray-300 bg-green-100 px-3 py-2 text-center">
                {row.present}
              </td>
              <td className="border border-gray-300 bg-green-100 px-3 py-2 text-center">
                {row.percentage}
              </td>
              <td className="border border-gray-300 bg-rose-100 px-3 py-2 text-center">
                {row.absent}
              </td>
            </tr>
          ))}
          <tr className="border-t-2 border-dashed border-sky-300">
            <td
              className="border border-gray-300 bg-slate-50 px-3 py-2 font-bold"
              colSpan={2}
            >
              TOTAL
            </td>
            <td className="border border-gray-300 bg-amber-200 px-3 py-2 text-center font-bold">
              {totals.total}
            </td>
            <td className="border border-gray-300 bg-green-200 px-3 py-2 text-center font-bold">
              {totals.present}
            </td>
            <td className="border border-gray-300 bg-green-200 px-3 py-2 text-center font-bold">
              {totals.percentage}
            </td>
            <td className="border border-gray-300 bg-rose-200 px-3 py-2 text-center font-bold">
              {totals.absent}
            </td>
          </tr>
        </tbody>
