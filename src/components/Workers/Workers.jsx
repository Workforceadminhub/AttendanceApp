...
import { getCachedFilterData, getFilterOptions, initializeFilterData } from "../../utils/filterCache";
...
export default function Workers() {
  ...
  const optionsAdmin = getAdminSelectOptions(isChurchAdmin || isSuperAdmin, team);
  ...
  const generateFallbackFilterOptions = () => {
    ...
    return {
      departments: [
        ...
        ...Array.from(allDepartments).sort().map(dept => ({ value: dept, label: dept }))
      ],
      teams: [
  ...
  const querySuperAdminWorkers = async (page = 1, limit = 20) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `HTTP ${response.status}: Failed to fetch workers`);
      }
  ...
  useEffect(() => {
    const loadFilterOptions = () => {
      ...
      if (cachedFilterData) {
        ...
        const accessToken = sessionStorage.getItem("accessToken");
        if (accessToken) {
          initializeFilterData(accessToken).then((filterData) => {
            if (filterData) {
  ...
  const updateDepartmentsForTeam = (selectedTeam) => {
    if (selectedTeam === "All") {
      ...
      if (teamData && Array.isArray(teamData.department)) {
        setAvailableDepartments([
          { value: "All", label: "All Departments" },
          ...teamData.department.sort().map(dept => ({ value: dept, label: dept }))
        ]);
      } else {
  ...
  const deleteWorker = async (workerId) => {
    ...
    try {
      ...
      const response = await fetch(`https://hchpk68xfh.execute-api.eu-west-1.amazonaws.com/api/super/admin/${workerId}/workers`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: 'Unknown error occurred' }));
        throw new Error(errorData.message || `HTTP ${response.status}: Failed to delete worker`);
      }
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {Array.isArray(data) && data.map((person, idx) => {
                        // Use API field names: firstname, lastname, or fallback to fullname split
                        const firstName = person.firstname || splitName(person.fullname).firstName;
                        const lastName = person.lastname || splitName(person.fullname).lastName;
                            ...
                            <tr key={person.id} className="hover:bg-gray-50">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <div className="flex space-x-2">
                                  ...
                                  <button
                                    onClick={() => handleEditWorker(person.id)}
                                ...
                                <td colSpan="8" className="px-6 py-4">
                                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    <div>
                                      ...
                                      <p className="mt-1 text-sm text-gray-900">{person.othername || "N/A"}</p>
                                    </div>
                                    <div>
                                      <label className="block text-sm font-medium text-gray-700">Email Address</label>
                                      <p className="mt-1 text-sm text-gray-900">{person.email || "N/A"}</p>
                                    </div>
                                    <div>
                                      <label className="block text-sm font-medium text-gray-700">Marital Status</label>
                                      <p className="mt-1 text-sm text-gray-900">{person.maritalstatus || "N/A"}</p>
                                    </div>
                                    <div>
                                      <label className="block text-sm font-medium text-gray-700">Birthday</label>
                                      <p className="mt-1 text-sm text-gray-900">{person.birthdate || "N/A"}</p>
                                    </div>
                                    <div>
                                      <label className="block text-sm font-medium text-gray-700">Age Range</label>
                                      <p className="mt-1 text-sm text-gray-900">{person.agerange || "N/A"}</p>
                                    </div>
                                    <div>
                                      <label className="block text-sm font-medium text-gray-700">Gender</label>
                                      <p className="mt-1 text-sm text-gray-900">{person.gender || "N/A"}</p>
                                    </div>
                                    <div className="md:col-span-2 lg:col-span-3">
                                      <label className="block text-sm font-medium text-gray-700">Address</label>
                                      <p className="mt-1 text-sm text-gray-900">{person.address || "N/A"}</p>
                                    </div>
                                    <div>
                                      <label className="block text-sm font-medium text-gray-700">Employment Status</label>
                                      <p className="mt-1 text-sm text-gray-900">{person.employment || "N/A"}</p>
                                    </div>
                                    <div>
                                      <label className="block text-sm font-medium text-gray-700">Occupation</label>
                                      <p className="mt-1 text-sm text-gray-900">{person.occupation || "N/A"}</p>
                                    </div>
                                  </div>
                                </td>
                              </tr>
                        ...
                        <tr>
                          <td colSpan="8" className="px-6 py-8 text-center text-gray-500">
                            <div className="flex flex-col items-center">
                              ...
                              <p className="text-lg font-medium text-red-900 mb-2">Data format error</p>
                              <p className="text-sm text-red-600">
                                The API response is not in the expected format. Please check the console for details.
                              </p>
                            </div>
                          </td>
                        </tr>
                ...
                <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-1">
                      {/* Previous Button */}
                      <button
                        onClick={() => querySuperAdminWorkers(pagination.page - 1, pagination.limit)}
                      ...
                      {(() => {
                        ...
                        for (let i = startPage; i <= endPage; i++) {
                          pages.push(
                            <button
                              key={i}
                              onClick={() => querySuperAdminWorkers(i, pagination.limit)}
                              className={`px-3 py-1 text-sm font-medium rounded-md ${
                                currentPage === i
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <ReactSelectDropdown
                  defaultValue={{
                    value: filters.team,
                    label: filterOptions.teams.find(t => t.value === filters.team)?.label || "All Teams",
                  }}
                  onChange={(selected) => handleFilterChange('team', selected?.value)}
              ...
              <div>
                ...
                <ReactSelectDropdown
                  defaultValue={{
                    ...
                    label: availableDepartments.find(d => d.value === filters.department)?.label || "All Departments",
                  }}
