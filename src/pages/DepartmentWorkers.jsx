...
export default function DepartmentWorkers() {
 ...
 <span>/</span>
 <span className="font-semibold text-ink-900">Workers</span>
 </>
 ) : (
 <span className="font-semibold text-ink-900">Workers</span>
 )
 ...
 className="inline-flex items-center px-4 py-2 text-sm font-medium text-ink-900 bg-white border border-ink-300 rounded-md hover:bg-cream"
 ...
 className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-ink-900 rounded-md hover:bg-black disabled:opacity-50"
 title={
 selectedVisibleCount > 0
 ...
 disabled={isDeleting}
 className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-brick rounded-md hover:bg-brick/80 disabled:opacity-50"
 >
 ...
 className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-forest rounded-md hover:bg-forest/80"
 >
 Add Worker
 </Link>
 <Link
 to={`${departmentUrl}/bulk-add`}
 className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-ink-900 rounded-md hover:bg-ink-800"
 >
 Bulk Add Workers
 </Link>
 </>
 ...
 value={selectedDepartmentFilter}
 onChange={(e) => setSelectedDepartmentFilter(e.target.value)}
 className="min-w-[220px] rounded-md border border-ink-300 bg-white px-3 py-2 text-sm focus:border-ink-900 focus:outline-none focus:ring-1 focus:ring-ink-900/10"
 >
 <option value="All">All</option>
 {departmentFilterOptions.map((d) => (
 <option key={d} value={d}>
 ...
 const canViewThisWorker =
 !isSubTeamAdmin && !isTeamAdmin && canAccessThisWorker;
 // View destination = attendance stats (read-only)
 const statsHref = `/worker/${worker.id}/attendance?department=${encodeURIComponent(worker.department || decodedDepartment || "")}&team=${encodeURIComponent(worker.team || "")}`;
 // Edit destination = the editable worker page
 const editHref = `/worker/${worker.id}?from=department:${encodeURIComponent(decodedDepartment)}`;
 const detailItems = [
 ...
 className={
 canViewThisWorker
 ? "cursor-pointer hover:bg-cream-200 transition-colors"
 : ""
 }
 >
 {canSelectWorkers && (
 ...
 <span className="text-ink-500">No additional details available.</span>
 )}
 </div>
 </td>
 </tr>
 ...
 <p className="text-ink-500">No workers found in this department.</p>
 )}
 </div>
