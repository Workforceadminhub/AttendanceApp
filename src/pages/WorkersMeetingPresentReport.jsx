...
export default function WorkersMeetingPresentReport() {
  ...
  const teamSummary = React.useMemo(() => {
    ...
    TEAM_STRUCTURE.forEach(({ teams: deptTeams }) => {
      deptTeams.forEach((t) => {
        counts[t] = { present: 0, absent: 0, confirmed: 0, notAttending: 0 };
      });
    });
    ...
    scopedRegistrations.forEach((r) => {
      ...
      if (!counts[resolved]) {
        counts[resolved] = { present: 0, absent: 0, confirmed: 0, notAttending: 0 };
      }
      if (r.is_present === true) {
        counts[resolved].present += 1;
      } else {
        counts[resolved].absent += 1;
      }
  ...
  const totalPresentCount = scopedRegistrations.filter((r) => r.is_present === true).length;
  ...
  const exportExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Workers Meeting Attendance");
    ...
    worksheet.columns = [
      ...
      { header: "Confirmation Status", key: "confirmed_status", width: 18 },
      { header: "Marked At", key: "marked_at", width: 22 },
    ];

    filtered.forEach((r, idx) => {
      const isPres = r.is_present === true;
      ...
      worksheet.addRow({
        ...
        confirmed_status: isConf ? "Confirmed" : isNot ? "Not Attending" : "No Response",
        marked_at: formatDate(r.marked_present_at || r.updated_at || r.created_at),
      });
    });
    ...
    saveAs(new Blob([buffer]), `workers_meeting_attendance_${MEETING_DATE}.xlsx`);
  };
  ...
  return (
    <div className="min-h-screen bg-cream">
      <Header title="Workers Meeting Attendance Report" />
      <Layout>
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Stat
            label="Total Present"
            value={loading ? "..." : totalPresentCount}
          />
          <Stat
            ...
            <table className="min-w-full divide-y divide-ink-200">
              ...
              <tbody className="divide-y divide-ink-200">
                {TEAM_STRUCTURE.map((row, idx) => {
                  ...
                  const data = teamSummary[teamName] || { present: 0, absent: 0 };
                  const pct = strength > 0 ? Math.round((data.present / strength) * 100) : 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`${td} text-forest font-semibold`}>{data.present}</td>
                      <td className={td}>{pct}% ({data.present}/{strength})</td>
                    </tr>
                  );
                })}
              </tbody>
                  ...
                  {loading ? (
                    ...
                    filtered.map((r, i) => (
                      <tr key={r.id} className="hover:bg-cream-100">
                        ...
                        <td className={td}>{formatDate(r.marked_present_at || r.updated_at || r.created_at)}</td>
                      </tr>
                    ))
                  )}
                </tbody>
